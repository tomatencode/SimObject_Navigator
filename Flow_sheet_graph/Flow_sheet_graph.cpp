...
using namespace std::literals::string_literals; // allow suffix ""s for
                                                // std::string literal

void SimObject_to_dot_input(const nlohmann::json &sim_object,
                            std::ostream &dot_input,
                            const std::set<std::string> &suppress_connectors);
...
void Flow_Sheet_Graph::exec() {
  ...
  if (config.contains("dot_graph_tool_pathname")) {
    dot_graph_tool_pathname =
        config.at("dot_graph_tool_pathname").get<std::string>();
  }
  ...
  char dot_input_fname[2048], dot_output[2048],
      image_file[2048]; /// \todo replace by std::filesystem operations
  std::tmpnam(dot_input_fname);
  // std::strcpy(dot_input_fname,
  // R"x(C:\Users\TTV032\AppData\Local\Temp\my_graph.txt)x"); /// \todo remove
  // later
  std::tmpnam(dot_output);
  std::tmpnam(image_file);
  // strcat(image_file, ".png"); /// \todo this is obviously not a valid temp
  // file, just for testing...

  // std::strcpy(image_file,
  // R"x(C:\Users\TTV032\AppData\Local\Temp\my_graph.png)x"); /// \todo remove
  // later
  ...
  auto command = std::string(dot_graph_tool_pathname) + " "s +
                 std::string(dot_input_fname) + " -Tpng -o "s +
...
void SimObject_to_dot_input(const nlohmann::json &sim_object,
  ...
  std::map<std::string, std::string>
      source; // connector_uuid -> source_unit_uuid one source per connector
              // (UUID)
  std::map<std::string, std::set<std::string>>
      sinks; // connector_uuid -> set of sink_unit_uuid
  try {

    for (const auto &[unit_uuid, unit] :
         sim_object.at("Flowsheet").get<nlohmann::json::object_t>()) {
      for (const auto &port_name : unit.at("Ports").items()) {
        bool is_out_ = is_out(port_name.key());

        for (const auto &connector_uuid :
             unit.at("Ports").at(port_name.key()).at("UUIDs")) {
          if (is_out_) {
  ...
  for (const auto &[connector_uuid, source_unit_uuid] : source) {
    if (suppress_connectors.contains(connector_uuid))
      continue;

    if (sinks.contains(connector_uuid)) {
      for (const auto &sink_unit_uuid : sinks.at(connector_uuid)) {
        dot_input << "\"" << get_unique_unit_name(sim_object, source_unit_uuid)
                  << "\" -> \""
                  << get_unique_unit_name(sim_object, sink_unit_uuid) << "\""
                  << "[label=\""
                  << get_unique_connector_name(sim_object, connector_uuid)
                  << "\"]\n";
      }
    } else {
...
void Flow_Sheet_Graph::update_connector_list() {
  ...
  if (first_call) {
    ...
    int i = 0;
    for (const auto &[connector_uuid, connector] :
         sim_object.at("Connectors").get<nlohmann::json::object_t>()) {
      ...
      connect(checkBox, &QCheckBox::clicked,
              [this, listWidgetItem]() { on_itemClicked(listWidgetItem); });
